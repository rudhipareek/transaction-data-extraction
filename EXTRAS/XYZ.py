...
def process_image():
    ...
    if file:
        try:
            ...
            # Refined regular expressions for extracting transaction data
            # Match various formats for date and time (e.g., '25 Jul 2024, 8:07 pm')
            pattern_date_time = r"(\d{1,2} [A-Za-z]{3} \d{4}),? (\d{1,2}:\d{2} [apm]{2})"

            # Match amount without ₹ symbol as well (e.g., 20000 or 20,000)
            pattern_amount = r"(₹?\s*\d{1,3}(?:,\d{3})*|\d{4,})"

            # Match UPI transaction and Google transaction IDs
            ...
            if match_amount:
                # Remove any ₹ symbols or spaces and convert to float
                transaction_amount = float(match_amount.group(0).replace("₹", "").replace(",", "").strip())
